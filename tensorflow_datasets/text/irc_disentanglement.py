...
def _parse_out_timestamps(raw_texts, day_str):
  """Parsing timestamps. Similar logic is implemented here:
...
class IrcDisentanglement(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager
    ...
    split_to_dir = {
        tfds.Split.TRAIN: os.path.join(data_dir, "train"),
        tfds.Split.VALIDATION: os.path.join(data_dir, "dev"),
        tfds.Split.TEST: os.path.join(data_dir, "test")
    }
    ...
    return [
        tfds.core.SplitGenerator(
            name=split,
            gen_kwargs={
                "day_to_paths": _get_day_to_paths(split_to_dir[split])
            },
        )
